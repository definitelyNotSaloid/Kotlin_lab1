...
fun sendWikiSearchRequest(request: String) : SearchResults? {
    ...
    val promise: CompletableFuture<HttpResponse<String>>
    val webResponse : HttpResponse<String>
    try {
        promise = client.sendAsync(webRequest, HttpResponse.BodyHandlers.ofString())
        ...
        webResponse = promise.get() as HttpResponse<String>
        ...
        if (e.cause is ConnectException)
            println("No internet connection. What did you expect?")
        else
            println("something gone horribly wrong...")
        return null
    }
